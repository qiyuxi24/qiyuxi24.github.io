...
> **维护状态（2026-09）**：站点在线地址 `https://qiyuxi24.github.io/`，当前为**低频维护**模式——它定位为个人展示与内容存档的落点，而非日常写作的场所。日常的笔记、想法与日记记录放在更轻量的工具里（如知识库 / 笔记应用），这里只保留值得沉淀与对外展示的内容。内容更新不追求频率，只发生在值得的时刻：有了想沉淀的文章、或到了需要完整对外展示的阶段（如求职 / 作品集），一次性补齐即可。技术栈与工具链保持原样，随时可以捡起来继续用。
